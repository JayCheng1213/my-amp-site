...
/* 🖼️ 照片切換時的輕微縮放與淡出淡入 */
.img-zoom-enter-active, .img-zoom-leave-active {
  transition: opacity 0.2s ease, transform 0.25s cubic-bezier(0.16, 1, 0.3, 1);
}
.img-zoom-enter-from { opacity: 0; transform: scale(0.97); }
.img-zoom-leave-to { opacity: 0; transform: scale(1.01); }

/* 📝 核心優化：小字專屬淡出淡入（Cross-fade）電路 */
.caption-cross-enter-active, .caption-cross-leave-active {
  transition: opacity 0.2s ease, transform 0.2s ease;
}
.caption-cross-enter-from {
  opacity: 0;
  transform: translateY(3px); /* 從下方微幅浮現 */
}
.caption-cross-leave-to {
  opacity: 0;
  transform: translateY(-3px); /* 往上方微幅隱去 */
}
</style>